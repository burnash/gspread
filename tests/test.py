...
class WorksheetTest(GspreadTest):
    ...
    def test_get_all_records(self):
        ...
        d0 = dict(list(zip(rows[0], rows[1])))
        d1 = dict(list(zip(rows[0], rows[2])))
        d2 = dict(list(zip(rows[0], rows[3])))
    ...
    def test_get_all_records_different_header(self):
        ...
        d0 = dict(list(zip(rows[2], rows[3])))
        d1 = dict(list(zip(rows[2], rows[4])))
        d2 = dict(list(zip(rows[2], rows[5])))
        self.assertEqual(read_records[0], d0)
        self.assertEqual(read_records[1], d1)
        self.assertEqual(read_records[2], d2)
